...
//				console.log(1457, 'renderCanvasData', renderCanvasData);
				...
				//OVERLAY相关
				var overlay = image.data ? image.data.elements["x60003000"] : null;
				if(overlay){
					var OverLayArr = getOverLayArr(image.data.byteArray,overlay.dataOffset,overlay.length);
					var originImageData = renderCanvasContext.getImageData(0, 0, image.width, image.height);
					var OverLayArrLen = OverLayArr.length
					for (var i = 0; i < OverLayArrLen; i++) {
						if(OverLayArr[i]){
							originImageData.data[i*4] = 255;
							originImageData.data[i*4 + 1] = 255;
							originImageData.data[i*4 + 2] = 255;
							originImageData.data[i*4 + 3] = 255;
						}
					}
					renderCanvasContext.putImageData(originImageData, 0, 0, 0, 0, image.width, image.height);
				}
				image.stats.lastPutImageDataTime = (0, _now2.default)() - start;

				return renderCanvas;
			}
			function getOverLayArr (byteArray, position, length) {
			  ...
			  for (var i = 0; i < length; i++) {
			    ...
			    if(Number(byte) > 0){
			    	var strTemp = byte.toString(2);
			    	var arrayTemp = [];
				    var lengthTemp = strTemp.length;
				    var missLen = 8 - lengthTemp;
				    for (var j = 0; j < missLen; j++) {
				    	arrayTemp.push(0)
				    }
				    for (var k = 0; k < lengthTemp; k++) {
				    	arrayTemp.push(Number(strTemp[k]))
				    }
				    for (var m = 8; m >0; m--) {
				    	result.push(arrayTemp[m-1])
				    }
			    }else{
				    var missLen = 8;
				    for (var j = 0; j < missLen; j++) {
		...
		(function(module, exports, __webpack_require__) {
			...
			exports.default = function(image, lut, canvasImageDataData) {
				...
				var storedPixelDataIndex = 0;
				var numPixels = pixelData.length;
				console.log(numPixels,2012)
				//console.log(minPixelValue,19561956,pixelData);
				// NOTE: As of Nov 2014, most javascript engines have lower performance when indexing negative indexes.
				// We have a special code path for this case that improves performance.  Thanks to @jpambrun for this enhancement
				start = (0, _now2.default)();
				if(minPixelValue < 0) {
		...
		(function(module, exports, __webpack_require__) {
			...
			exports.default = function(image, viewport, invalidated) {
				...
				if(image.cachedLut !== undefined && image.cachedLut.windowCenter === viewport.voi.windowCenter && image.cachedLut.windowWidth === viewport.voi.windowWidth && (0, _lutMatches2.default)(image.cachedLut.modalityLUT, viewport.modalityLUT) && (0, _lutMatches2.default)(image.cachedLut.voiLUT, viewport.voiLUT) && image.cachedLut.invert === viewport.invert && invalidated !== true) {
					//console.log(image,viewport,52335233);
					return image.cachedLut.lutArray;
				}
